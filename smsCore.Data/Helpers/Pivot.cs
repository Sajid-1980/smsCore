...
public class Pivot
{
    ...
    public DataTable PivotData(string DataField, AggregateFunction Aggregate, string[] RowFields, string[] ColumnFields,bool columnOrder=true)
    {
        DataTable dt = new DataTable();
        string Separator = ".";
        ...
        // Gets the list of columns .(dot) separated.
        var ColList = columnOrder ? (from x in _SourceTable.AsEnumerable()
        ...
        foreach (var RowName in RowList)
        {
            try
            {
                ...
                foreach (var col in ColList)
                {
                   ...
                   // System.Windows.Forms.MessageBox.Show("this is value "+row[col.Name.ToString()].ToString());
                }
                dt.Rows.Add(row);
            }
            catch { }
    ...
    public DataTable PivotData(string DataField, AggregateFunction Aggregate, string[] RowFields, string[] ColumnFields,string UniqueID)
    {
        DataTable dt = new DataTable();
        string Separator = ".";
        ...
        foreach (var RowName in RowList)
        {
            try
            {
                ...
                //strFilter = strFilter.Substring(5);
                strFilter = UniqueID + " = '" + RowName[UniqueID] + "'";
                foreach (var col in ColList)
                {
                    string filter = strFilter;
                    string[] strColValues = col.Name.ToString().Split(Separator.ToCharArray(), StringSplitOptions.None);
    ...
    private object GetData(string Filter, string DataField, AggregateFunction Aggregate)
    {
        try
        {
           ...
           // System.Windows.Forms.MessageBox.Show("This is count " + FilteredRows.Count().ToString() + "\nFilter: " + Filter);
    ...
    {
        return (objList.Count() == 0) ? null : objList.First().ToString() == string.Empty ? "na" : objList.First();
    }
